...
> Retoma el trabajo de este repo. La fuente de verdad: `docs/CMS_ROADMAP.md` y `HANDOFF.md` (roadmap CMS **completo al 100%** — 20/20 fases originales A1–E3 certificadas, cerrada D2 el 2026-08-01 + backlog del punto 5: B7/B8/B9/E4/C1-2/gestos). Revisa `git status` y `git log --oneline -15`: debe estar limpio; si hay trabajo sin commitear, verifícalo (tsc/lint/tests) y commiétalo antes de seguir. **Entornos**: dev diario con Supabase LOCAL en podman (`make db-local-start`, esquema+seeds aplicados; Studio GUI :54323, Mailpit :54324; flip de `.env.local` con `make db-local-on/off` — respaldo en `.env.local.nube-backup`; runbook en `docs/OPERATIONS.md` § «Supabase LOCAL del día a día»; ANTES de tocar la nube — migraciones/seeds/invalidar caché en prod — corre `db-local-off`). CI/Nightly con Supabase local en el runner (A3). Disciplina por tarea: implementación → tsc + lint + prettier + tests focal → commit atómico en español → push a develop → vigilar CI verde → certificar en el documento que corresponda. Staging Supabase Free `lucams-stg` **ya creado** (2026-08-01 — previews de Vercel aislados de prod vía env vars scope Preview; runbook `docs/OPERATIONS.md` § «Environments») y el **catálogo real quedó sincronizado PRD→STG** (612 productos, 979 campos CMS, redirects, plantillas, 43 cupones reales). El mismo día **PRD quedó limpio de datos de prueba** (pedidos/clientes/cotizaciones/carts/designs/reviews/cupones test/admins de specs/telemetría borrados; backup verificado en `tmp/backups/prod-pre-cleanup-20260801.dump`; se conservaron catálogo, CMS, redirects, plantillas, `AdminActionLog` y el único admin real — changelog en OPERATIONS). **Pendiente ÚNICO de dashboards: Lucy cambia la Production Branch de Vercel de `develop` a `production`** (Settings → Git — la API pública no lo permite); sin ese flip, cada push a `develop` despliega en vivo. Smoke post-release: `release-check-a1.spec.ts` con PLAYWRIGHT_BASE_URL a prod.
