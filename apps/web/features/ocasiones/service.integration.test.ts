...
describe.skipIf(!hasDb)("ocasiones/service — integración DB (OcasionTag, PLAN_CATALOG_V2 1.5/2.10/3.4)", { timeout: T }, () => {
  beforeAll(async () => {
  ...
  describe("listOcasionTags — filtro status", () => {
    it("status 'active' devuelve solo isActive=true", async () => {
      ...
      const active = await makeTag({ slug: `${token}-a`, name: `${token}-Activo`, isActive: true });
      const inactive = await makeTag({ slug: `${token}-i`, name: `${token}-Inactivo`, isActive: false });
  ...
  describe("linkProductToOcasion / unlinkProductFromOcasion", () => {
    ...
    it("unlink de un par inexistente lanza P2025 (delete sin match)", async () => {
      const tag = await makeTag();
      await expect(
        unlinkProductFromOcasion(sharedProductId, tag.id),
      ).rejects.toMatchObject({ code: "P2025" });
    });
  ...
  describe("getProductsForOcasion", () => {
    ...
    it("INCLUYE asociaciones cuyo producto fue soft-deleted (el include NO filtra deletedAt)", async () => {
      ...
      await prisma.product.update({ where: { id: tmp.id }, data: { deletedAt: new Date(), isActive: false } });
