...
function assertServerRenderable(unit: UnitTemplate, slots: Slot[], includeText: boolean): void {
  ...
  for (const l of unit.layers) {
    ...
    if (l.type === "text") {
      ...
      // Ola 23 (Lucy 2026-09-08) — regla global de placeholders: el texto por defecto de
      // una capa EDITABLE ("Escribe tu mensaje", "@tu_usuario"…) es solo la guía del
      // Estudio y NUNCA se imprime. Sin override del cliente no hay contenido que
      // dibujar → el slot se queda en el tier sharp en vez de caer al canvas solo por
      // el placeholder (el tier canvas tampoco imprimiría nada: renderTextLayer).
