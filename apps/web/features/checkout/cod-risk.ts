 ...
 *
 * Este módulo evalúa el riesgo por IDENTIDAD (teléfono/email), no por IP: velocidad, pedidos COD en
 * vuelo, devoluciones previas y tope para clientes sin historial. Si dispara, se BLOQUEA el COD (el
 * cliente puede pagar en línea) — nunca se revela la regla exacta (para no darle el mapa al abusador).
 *
...
export async function assessCodRisk(orderId: string): Promise<CodRiskResult> {
  try {
    const order = await prisma.order.findUnique({
      where: { id: orderId },
      select: { id: true, email: true, phone: true, total: true },
    });
    ...
    if (!email && !phone) return { allowed: true };
    ...
    const [recentCod, outstandingCod, deliveredHistory, priorReturned] = await Promise.all([
    ...
    if (priorReturned > 0) code = "prior_return";
