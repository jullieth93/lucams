...
const RUN = `ITESTREDIR${Date.now()}${Math.floor(Math.random() * 1e6)}`.toUpperCase();
...
describe.skipIf(!hasDb)("redirects/service — integración DB (UrlRedirect admin 301/302)", () => {
  afterAll(async () => {
    ...
    await prisma.urlRedirect.deleteMany({ where: { fromPath: { startsWith: `/${RUN}` } } });
  });
  ...
  describe("lookupActiveRedirect", () => {
    ...
    it("el match de fromPath es case-sensitive (no normaliza casing en lookup)", async () => {
      // normalizePath en createRedirect NO hace lowercase (a pesar del comentario
      // del código). Verificamos que el lookup respeta el casing almacenado.
      const from = `/${RUN}/CaseSensitive${++seq}`;
      await seedRedirect({ fromPath: from, toPath: "/x" });

      expect(await lookupActiveRedirect(from)).not.toBeNull();
      expect(await lookupActiveRedirect(from.toLowerCase())).toBeNull();
  ...
  describe("incrementRedirectHit", () => {
    ...
    it("avanza lastHitAt en hits sucesivos (monotónico no-decreciente)", async () => {
      const from = nextPath();
      await seedRedirect({ fromPath: from, hitCount: 0, lastHitAt: new Date(Date.now() - 60_000) });
