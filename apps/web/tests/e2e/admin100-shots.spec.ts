...
const MODULES = ["reclamos", "mensajes", "mayorista", "materiales", "costos", "metricas", "performance"];
const ROUTES = [...MODULES.map((m) => `/admin/${m}`), "/admin/canales/tienda"];

test.setTimeout(240_000);

test.beforeAll(async () => {
  const { data, error } = await service.auth.admin.createUser({ email: ADMIN_EMAIL, password: ADMIN_PASSWORD, email_confirm: true });
  if (error || !data.user) throw new Error(`no auth user: ${error?.message}`);
  supabaseUserId = data.user.id;
  const admin = await prisma.adminUser.create({ data: { supabaseUserId, email: ADMIN_EMAIL, role: "SUPERADMIN", isActive: true } });
  adminId = admin.id;
});
