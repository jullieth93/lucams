...
// Owner 2026-09-24, v2 tras prueba STG — Estudio configurable por producto
// (admin → Avanzado): tamaño BASE del lienzo (canvasBaseScale — el "100%" del
// cliente) y override FORZADO de columnas de la grilla (gridColsOverride),
// ambos guardados en el personalizationSchema.
describe("resolveCanvasBaseScale — tamaño base del lienzo por producto", () => {
  it("sin configurar (null/undefined/NaN) → 1 (tamaño estándar)", () => {
    expect(resolveCanvasBaseScale(undefined)).toBe(1);
    expect(resolveCanvasBaseScale(null)).toBe(1);
    expect(resolveCanvasBaseScale(NaN)).toBe(1);
  });

  it("valor configurado dentro del rango → se respeta", () => {
    expect(resolveCanvasBaseScale(1.5)).toBe(1.5);
    expect(resolveCanvasBaseScale(0.5)).toBe(STAGE_ZOOM_MIN);
    expect(resolveCanvasBaseScale(2.5)).toBe(STAGE_ZOOM_MAX);
  });

  it("fuera de rango (JSON libre del producto) → clamp a [MIN, MAX]", () => {
    expect(resolveCanvasBaseScale(0.1)).toBe(STAGE_ZOOM_MIN);
    expect(resolveCanvasBaseScale(99)).toBe(STAGE_ZOOM_MAX);
  });
});

describe("clampGridColsOverride — override FORZADO de columnas por producto", () => {
  ...
  it("clamp [1..6]: 0 → 1 y 12 → 6 (el override NO se capea contra resolveMaxCols — fuerza)", () => {
    expect(clampGridColsOverride(0)).toBe(1);
    expect(clampGridColsOverride(12)).toBe(6);
    // Contrato v2 (STG): el clamp devuelve el valor pedido aunque el ancho
    // objetivo dé menos columnas — el cálculo de resolveMaxCols ya no capea el
    // override (las guardas móvil/slots/piso viven en el grid).
    const maxColsNarrow = resolveMaxCols({
      containerWidth: 950,
      isCalendar: false,
      gridCols: 3,
      slotCount: 6,
    });
    expect(maxColsNarrow).toBe(2); // el automático daría 2…
    expect(clampGridColsOverride(3)).toBe(3); // …pero el override fuerza 3.
  });
});
