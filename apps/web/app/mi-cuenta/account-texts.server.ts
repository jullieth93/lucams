/*
 * getAccountTexts — resuelve SERVER-SIDE los textos CMS del área de cliente
 * /mi-cuenta (roadmap B9): UNA query por prefijo `account.*` (kind BLOCK,
 ...
 * exacto pre-CMS (REGLA DE ORO) — mismo patrón que getAuthTexts (B7).
 *
...
import { ACCOUNT_TEXT_KEYS, DEFAULT_ACCOUNT_TEXTS, type AccountTexts } from "./account-texts";

async function fetchAccountBodies(): Promise<Record<string, string>> {
  try {
...
const cachedAccountBodies = unstable_cache(fetchAccountBodies, ["account-texts"], {
  tags: ["cms"],
  revalidate: 3600,
});
...
async function loadBodiesSafe(): Promise<Record<string, string>> {
  try {
    return await cachedAccountBodies();
  } catch (err) {
...
export async function getAccountTexts(): Promise<AccountTexts> {
  const bodies = await loadBodiesSafe();
  // structuredClone: nunca mutar el DEFAULT compartido entre requests.
  const texts = structuredClone(DEFAULT_ACCOUNT_TEXTS) as AccountTexts;
  for (const [path, cmsKey] of Object.entries(ACCOUNT_TEXT_KEYS)) {
