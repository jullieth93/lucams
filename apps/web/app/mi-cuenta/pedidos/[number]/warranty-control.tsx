...
export function WarrantyControl({ item }: { item: WarrantyItem }) {
  ...
  if (!item.eligible) {
    if (item.reason === "OUT_OF_WARRANTY") {
      return <p className="text-brand-muted mt-1 text-[11px]">Fuera del periodo de garantía.</p>;
    }
    return null;
  }
    ...
    <details className="mt-1.5">
      ...
      <form action={action} className="mt-2 space-y-2">
        ...
        <textarea
          ...
          minLength={10}
          maxLength={2000}
          placeholder="Cuéntanos qué pasó (ej. el imán se despegó, llegó rayado…)"
        ...
        {endsAt && (
          <p className="text-brand-muted text-[10px]">
            Cubierto por garantía legal hasta el {dateFmt.format(endsAt)}.
          </p>
        )}
        ...
        >
          {pending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Enviar reclamo"}
        </button>
      </form>
    </details>
  );
