...
try {
  const settings = await prisma.siteSetting.findMany({
    where: { key: { in: ["BUSINESS_NIT","PICKUP_CITY","PICKUP_DEPARTMENT","PICKUP_ADDRESS","PICKUP_PHONE","PICKUP_CONTACT_NAME"] } },
    select: { key: true, value: true, category: true },
  });
  for (const s of settings) report.sitesettings[s.key] = { present: !!s.value, len: s.value?.length ?? 0, category: s.category };
  const missing = ["BUSINESS_NIT","PICKUP_CITY","PICKUP_DEPARTMENT","PICKUP_ADDRESS","PICKUP_PHONE","PICKUP_CONTACT_NAME"].filter(k => !report.sitesettings[k]);
  report.sitesettings._missing = missing;
} catch (e) { report.sitesettings._error = e.message; }
...
} catch (e) { report.aveonline._error = e.message; }
  ...
  if (report.aveonline.parsed?.hasToken) {
    ...
    if (token && idempresa) {
      ...
      try {
        ...
        report.aveonline.transportadoras.list = j?.transportadoras?.map(t => ({ id: t.id, text: t.text })) ?? null;
      } catch {}
    }
  }
} catch (e) { report.aveonline.transportadoras_error = e.message; }
...
try {
  if (report.aveonline.parsed?.hasToken) {
    ...
    const idempresa = idMatch ? Number(idMatch[1]) : null;
    if (token && idempresa) {
      const r = await fetch("https://app.aveonline.co/api/nal/v1.0/generarGuiaTransporteNacional.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          tipo: "cotizarDoble",
          ...
          productos: [{ alto: 10, ancho: 15, largo: 3, peso: 0.5, unidades: 1, nombre: "set-6", valorDeclarado: 35000 }],
          contraentrega: 0,
      ...
      try {
        ...
        report.aveonline.cotizarDoble.oks = oks.map(c => ({ carrier: c.nombreTransportadora, total: c.total, dias: c.diasentrega }));
        report.aveonline.cotizarDoble.errs = errs.map(c => ({ carrier: c.nombreTransportadora, code: c.numbererror, msg: c.dataerror?.slice(0, 120) }));
      } catch {}
    }
  }
} catch (e) { report.aveonline.cotizarDoble_error = e.message; }
    ...
    if (token && idempresa) {
      const r = await fetch("https://app.aveonline.co/api/nal/v1.0/generarGuiaTransporteNacional.php", {
        ...
        body: JSON.stringify({
          ...
          origen: "Bogotá",
          destino: "Medellín",
          productos: [{ alto: 10, ancho: 15, largo: 3, peso: 0.18, unidades: 1, nombre: "set-6", valorDeclarado: 35000 }],
          contraentrega: 0,
          idasumecosto: 0,
          plugin: "apiave",
        }),
      ...
      try { report.aveonline.quote.parsed = JSON.parse(text); } catch {}
    }
  }
} catch (e) { report.aveonline.quote_error = e.message; }
...
try {
  if (report.aveonline.parsed?.hasToken) {
    ...
    const productos = [{ alto: 10, ancho: 15, largo: 3, peso: 0.5, unidades: 1, nombre: "set-6", valorDeclarado: declared }];
    const formatCity = (city, dept) =>
      `${city.normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/\bD\.?\s*C\.?\b/gi,"").trim().toUpperCase()}(${dept.normalize("NFD").replace(/[̀-ͯ]/g,"").trim().toUpperCase()})`;
    const r = await fetch("https://app.aveonline.co/api/nal/v1.0/generarGuiaTransporteNacional.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        tipo: "cotizarDoble", access: "", token, idempresa,
        origen: formatCity("Bogotá", "Cundinamarca"),
        destino: formatCity("Medellín", "Antioquia"),
        productos, contraentrega: 0, contraentregaPayment: 0, valorrecaudo: 0,
        valorMinimo: 0, idasumecosto: 0, plugin: "lucamsshop",
      }),
    });
    ...
    report.providerSimulation = {
      ...
      failedSamples: failed.slice(0, 3).map(c => ({ carrier: c.nombreTransportadora, code: c.numbererror })),
    };
  }
} catch (e) { report.providerSimulation_error = e.message; }
...
try {
  const variants = await prisma.productVariant.findMany({
    ...
    take: 5,
    include: { product: { select: { slug: true, physicalSpecs: true, personalizationKind: true } } },
  });
...
} catch (e) { report.dims._error = e.message; }
