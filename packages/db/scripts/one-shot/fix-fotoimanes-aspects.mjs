 ...
 *
 * Idempotente. Uso: DATABASE_URL=$DIRECT_URL node scripts/one-shot/fix-fotoimanes-aspects.mjs
 */
...
assertDestructiveAllowed("fix-fotoimanes-aspects.mjs");
...
async function setAspect(slug, aspectRatio) {
  ...
  const sc = {
    ...(prod.personalizationSchema && typeof prod.personalizationSchema === "object"
      ? prod.personalizationSchema
      : {}),
    aspectRatio,
  };
...
async function main() {
  ...
  const tpl = await prisma.personalizationTemplate.findFirst({
    where: { slug: "photo-pack-polaroid-instagram" },
  ...
  const libre = await prisma.personalizationTemplate.findFirst({
    where: { slug: "libre-photo-pack" },
  ...
  const cuad = await prisma.product.findFirst({
  ...
  if (cuad) {
    ...
    const found = await prisma.personalizationTemplate.findFirst({
      where: { slug: "foto-cuadrado-simple" },
    });
    const tdata = {
      productId: cuad.id,
      kind: "PHOTO_PACK",
      name: "Cuadrado simple",
      description: "Fotoimán cuadrado — tu foto a todo el imán.",
    ...
    if (found) {
      ...
      await prisma.personalizationTemplate.create({
        data: { slug: "foto-cuadrado-simple", ...tdata },
      });
