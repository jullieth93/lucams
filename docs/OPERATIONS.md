...
- ⚠️ **PENDIENTE (hallazgo auditoría 2026-08-01 noche): la `SUPABASE_SECRET_KEY` de `lucams-stg` es inválida** (401 en auth-admin/PostgREST y "Invalid Compact JWS" en Storage — es un valor errado de nacimiento, no la llave real del proyecto; la `sb_publishable` sí es válida). Impacto: en los previews fallan uploads a Storage y operaciones con service client (admin) — el storefront público funciona (Prisma + publishable). **Fix (Lucy, 5 min):** dashboard Supabase → proyecto `lucams-stg` → Settings → API Keys → copiar la `sb_secret_…` real → actualizar `SUPABASE_SECRET_KEY` en `.env.stg` (VM) y en Vercel scope Preview → redeploy del preview → verificar `/api/health/storage` = ok. No hay token de Management API en la VM para hacerlo por CLI.
