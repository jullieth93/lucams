...
**Avance del carril código (2026-07-13):** **P1.1 E2E/axe en CI** ✅ (`16a6b83` — job `e2e`: build prod + Postgres + seed → next start → smoke/a11y/axe/compra en cada PR; visual/estudio/admin-auth documentados como follow-up). **P1.4 Vista 3D en nevera** ✅ (`bb6cc59` — diferenciador #1: `fridge-3d-view.tsx` con React Three Fiber, self-contained sin assets externos por CSP, lazy-load; botón "Ver en 3D" + modal accesible en el editor; texturas por-slot recortadas a la silueta; **verificado en navegador**. Deps three+R3F+drei autorizadas por Lucy). Lucy aclaró: "orden = secuencia, NO exclusión" → AI + R2 + DIAN también autorizados, se secuencian.

**Próximo del carril código (secuencia):** P1.2 cobertura (medir + ratchet, sin dep, `@vitest/coverage-v8` ya está — ojo RAM al correr --coverage) → P2 backoffice (SupportTicket UI, WarrantyClaim, razones de auditoría en inventario/transiciones) → P1.5 asistente IA (`@anthropic-ai/sdk` autorizado; **con rate-limit + cache** porque la API factura por uso) → P2 backups R2 (`@aws-sdk/client-s3`) → DIAN InvoiceProvider (adapter + flujo; el provider real espera decisión de Lucy con contador). E2E de compra pagada real (Wompi sandbox) sigue pendiente por fragilidad de red. Ver artifact ruta-produccion + journal en subagents/workflows/wf_e7682d03-569.
