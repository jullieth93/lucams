...
**Próximos autónomos (mismo orden):** P0-2 moderación mínima (consentimiento por-subida + gate de revisión manual antes del render de producción), P1 ledger de conciliación financiera COD, luego P2 (CORS regex al sufijo de equipo, ACAO coupons, health/all baseUrl de fuente confiable, SLOs, anti-abuso COD, E2E sandboxes, Lighthouse). **Carril de Lucy (en paralelo, no bloquea código):** abogado (figura legal + NIT), contador (DIAN), provisionar Wompi/Aveonline prod + dominio + R2 + DNS de correo, agendar crons en pg_cron.
