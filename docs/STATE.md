...
**Diferido con razón (necesita pase dedicado):** editar-diseño-desde-carrito (complejo: remap de asset-ids en canvas vs edición-en-sitio con riesgo de abandono; camino crítico del carrito → requiere tests de integración) · admin-login/MFA/Estudio en el gate E2E por-PR (infra CI) · rls-matrix de comportamiento en CI (necesita stack Supabase) · JSON-LD de categoría enriquecido · palancas de ingreso P3 (recuperación de carrito, email post-entrega, wishlist).
