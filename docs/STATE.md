...
**🏭 PLAN MAESTRO (ADR-062) — carril autónomo iniciado.** Lucy pidió priorizar los launch-blockers verificables. Hecho:
...
- **⏭️ PRÓXIMO AUTÓNOMO:** **ledger de conciliación financiera COD (P1)** — feature grande (modelo + migración + admin UI + lógica de conciliación de efectivo contraentrega). Luego SLOs, anti-abuso COD (parcial: rate-limit COD ya existe), E2E sandboxes, Lighthouse.
- **Deuda de pruebas GUI (Lucy, navegador):** descarga producción + ZIP en /admin/pedidos, grid por-slot en /admin/moderacion, selector de año, lazy-mount, **calendario 3D**.
