...
- **✅ LEDGER DE CONCILIACIÓN COD (ADR-064)** — la dimensión más débil del audit (COD antifraude+conciliación 55). Workflow de comprensión (5 agentes) → diseño: modelo `CodReconciliation` operacional, creado SOLO al resolver (Lucy marca remitido/discrepancia); "por remitir" se DERIVA de órdenes COD DELIVERED sin fila REMITTED. Cero cambios en la saga. Schema+migración (a mano, RLS auto vía event trigger, verificado `relrowsecurity=t`) + service (`features/orders/cod-reconciliation.ts`, idempotente por `orderId @unique`) + `/admin/finanzas/conciliacion` (SUPERADMIN, KPIs + acciones remesa/discrepancia, montos en pesos) + sidebar Finanzas→grupo + aviso en Finanzas + `codPendingRemitCop`/`codDiscrepancies` en el resumen diario. Antifraude: hace visible el efectivo que el mensajero ya cobró y debe. Certificado: tsc+lint+prettier+build + 2 unit + 6 integración (cleanup scoped). Pendiente: review adversarial + commit/push/CI.
- **⏭️ PRÓXIMO AUTÓNOMO:** SLOs cuantitativos, anti-abuso COD (parcial: rate-limit COD ya existe), E2E sandboxes, Lighthouse.
- **Deuda de pruebas GUI (Lucy, navegador):** descarga producción + ZIP en /admin/pedidos, grid por-slot en /admin/moderacion, selector de año, lazy-mount, **calendario 3D**, y **/admin/finanzas/conciliacion** (marcar remesa/discrepancia de un COD entregado).
