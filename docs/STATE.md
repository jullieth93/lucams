...
**⏳ EN CURSO — Plan de 3 categorías del Estudio (2026-07-12 cont., ADR-057).** Lucy pidió ejecutar TODO el plan ([PLAN_CATEGORIAS_ESTUDIO.md](PLAN_CATEGORIAS_ESTUDIO.md)) y certificar cada fase. Progreso:
- **Fase A (render server-side) — COMPLETA y certificada.** A0 (`bd7aa90`, persistir encuadre), A1a (`edff931`+`768a5bd`, sharp para foto pura), A1b (`01bfc0a`+`c566c9a`, @napi-rs/canvas para texto/marcos). Cada una con revisión adversarial (workflow) y fixes conservadores. Tier en finalizeDesign: sharp→canvas→cliente, con fallback seguro (nunca rompe producción). Filtros siguen en fallback al cliente (filtro exacto de Konva). Ver [PLAN_CATEGORIAS_ESTUDIO.md] + bitácora.
- **Fase B (Separadores) — CORE completo, B2 pendiente.** Unificados los 2 productos inactivos en UNO vendible "Separadores para Libros" (`separadores-libros`, `a8c12d9`): PHOTO_PACK, variantes forma (cuadrado 1:1 / rectangular 5:14) × cantidad (1/3/5), activado. Plantillas dedicadas por forma (`2bbfbff`) → el shape rectangular renderiza tall (WYSIWYG); ruteo por aspecto certificado. Subida-por-cantidad funciona vía el editor de foto. Script `make seed-separadores`. **Pendiente B2:** galería de diseños PREDISEÑADOS elegibles por slot en el editor (la visión de Lucy: prediseñados + subir imagen).
- **Fase C (Fotoimanes) — PENDIENTE.** Pulir por forma (círculo/corazón/cuadrado WYSIWYG) + Polaroid con texto. El flujo foto ya funciona; A1a/A1b ya renderizan estos en el servidor.
- **Fase D (Calendarios) — PENDIENTE.** Editor a medida: 12 meses etiquetados + año + grilla WYSIWYG (hoy se aplana a 12 fotos sueltas). Nueva superficie "calendar". Mayor esfuerzo.

**Reanudación:** seguir con B2 (galería prediseñados) → C (fotoimanes) → D (calendarios). Todo en `develop`, verificado a cada commit (tsc+lint+tests+build; revisión adversarial en cambios de ruta de dinero/producción).
