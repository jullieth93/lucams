...
- **✅ T1 hecho** (`64eea43`+`d86d887`) — `lib/storage.getProductionAssetSignedUrls` (firma batch del bucket privado, verificada en vivo) + `getOrder` trae productionUrls+moderationStatus + links de descarga por slot en /admin/pedidos con aviso si el diseño no está aprobado. **GUI test pendiente de Lucy.**
- **⏭️ PRÓXIMO — CAL1 (P0, large):** construir el calendario REAL — compositor server-side (@napi-rs/canvas, ya presente en `features/personalization/production-render-canvas.ts`) que hornée alrededor de cada foto: nombre del mes + año + **grilla de días**; y que el editor MUESTRE ese layout (hoy las etiquetas de mes son overlay DOM en `studio-slot.tsx:608-616`, excluidas del toDataURL). `production-render.ts`/`production-render-canvas.ts` NO tienen rama de calendario hoy. Pasos: (1) helper puro `calendarMonthGrid(year, monthIndex)` + tests; (2) compositor de página de mes en canvas; (3) integrar en `finalizeDesign` para CALENDAR_PHOTO_MONTH; (4) preview del layout en el editor.
- **Luego (mismo orden del Artifact):** P1 (T3 snapshot limpio, T2 moderación con zoom por-slot, T7 hoja de armado, T4 blindar boot del editor, T5 lazy-mount, D1 superficies fantasma phrase/event/logo, FOTO1 corte corazón/círculo, FOTO2 polaroid + texto inglés en `ig_post.svg`, CAL2 año, CAL3 plantillas calendario, NOM1) → P2 inmersivo (CAL4 mockup navegable de página de mes, FOTO3 flat-lay de regalo, FOTO4, NOM2, SEP1) → P3.
- **Carril de Lucy (contenido):** arte de meses, marcos temáticos (corazón/círculo), tamaños, fichas de letras. Rinde después de que el código deje de romperse al curar plantillas reales.
