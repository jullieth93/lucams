...
**P1.5 Asistente IA de sugerencias** ✅ (`1ac31cb`, ADR-058): el 2º diferenciador. Adaptador **proveedor-agnóstico** (`AiProvider`) + **Gemini** vía REST (sin dep npm, key server-only, sin tocar CSP) + **fallback entre modelos** (flash-lite→flash) + rate-limit + action + UI (botón "Ideas" en el editor → ocasión → frase/color/composición/tip). Se eligió Gemini sobre Claude por free-tier (mandato #2) tras evaluación de costo-beneficio con Lucy. Falla-seguro (nunca rompe el editor). 6 tests (incl. fallback). **Con esto ambos diferenciadores (3D + IA) están listos.** ACCIÓN HUMANA: setear `GEMINI_API_KEY` + confirmar IDs de modelo.

**Próximo del carril código (secuencia restante):** (`@anthropic-ai/sdk` autorizado; **con rate-limit + cache** porque la API factura por uso) → P2 backups R2 (`@aws-sdk/client-s3`) → P1.2 cobertura (medir + ratchet, ojo RAM al correr --coverage con el dev server arriba) → DIAN InvoiceProvider (adapter + flujo; el provider real espera decisión de Lucy con contador). E2E de compra pagada real (Wompi sandbox) sigue pendiente por fragilidad de red. Ver artifact ruta-produccion + journal en subagents/workflows/wf_e7682d03-569.
