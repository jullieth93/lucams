...
**Próximo del carril código (autónomo, pendiente):** stock "Agotado" + checkout + JSON-LD, editar-diseño→DRAFT, cache de public-service, pg_cron versionado, SEO categorías, gap RLS en CI, tests proxy/E2E admin. **Carril humano (Lucy):** identidad legal (persona natural vs S.A.S. + abogado), DIAN (contador/copy FAQ), R2 (provisionar bucket), DROP modelos muertos. **DIAN InvoiceProvider** espera decisión de proveedor. Ver ADR-060 + artifact.
