...
Inventario de `apps/web/features/emails/templates/` (verificado 2026-09-12): **26 templates**
(antes 23; +3 de la remediación 360° del 2026-09-11/12: order-payment-declined, order-returned,
...
| **order-payment-declined**   | Webhook Wompi `DECLINED`/`ERROR` con la orden aún `PENDING_PAYMENT` (N-22a, 2026-09-11) | Avisa que el pago NO fue aprobado y el pedido sigue vivo/reintentable (antes noop silencioso). Emisor: `sendOrderPaymentDeclined` (`features/orders/emails.ts`), best-effort total. Idempotente POR TRANSACCIÓN (`idempotencyKey: <number>-payment-declined-<txId>`) + anti-spam POR ORDEN: claim atómico sobre `Order.paymentFailedNotifiedAt` con cooldown 6h (un claim fallido NO se libera: prioridad no-spam). Solo si sigue `PENDING_PAYMENT` + WOMPI. |
| **order-cancelled**          | Cancelación manual admin, o `CANCELLED` post-pago (FULFILLING/SHIPPED)                  | Con motivo opcional.                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **refund-issued**            | `REFUNDED` (VOIDED post-pago, retracto aprobado, etc.)                                  | Monto + motivo del reembolso. Desde N-17 (2026-09-11) el admin debe confirmar antes el dinero devuelto (checkbox bloqueante; evidencia `refundMoneyConfirmedAt/By`).                                                                                                                                                                                                                                                                                         |
| **order-returned**           | Webhook Aveonline `RETURNED`/`EXCEPTION` (N-22b, 2026-09-11)                            | "Tu pedido viene de vuelta, te contactamos" (sin promesas). Emisor: `notifyOrderReturned` (`features/orders/emails.ts`), best-effort. Email al cliente con `idempotencyKey: <number>-returned` + notificación in-app admin con la acción esperada (reenvío / reembolso / reposición — nada automático; dedupKey `order-returned-<orderId>`).                                                                                                                 |
...
| **support-ticket-received**  | Cliente crea ticket de soporte                                                          | Confirmación al cliente (declara que la respuesta es humana por email).                                                                                                                                                                                                                                                                                                                                                                                      |
| **support-ticket-internal**  | Ticket de soporte → buzón interno                                                       | Notificación al negocio (Reply-To al cliente).                                                                                                                                                                                                                                                                                                                                                                                                               |
| **support-ticket-closed**    | Admin cierra el ticket en `/admin/soporte` (N-14, 2026-09-11)                           | Aviso de cierre del ciclo. Emisor: `sendTicketClosedEmail` (`features/support/admin-service.ts`), best-effort; solo si el estado previo NO era CLOSED; `idempotencyKey: support:closed:<ticketId>`.                                                                                                                                                                                                                                                          |
