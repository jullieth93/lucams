...
## ADR-100 — Paquete coherencia Admin↔Front↔Estudio (2026-09-14): escaleras históricas restauradas, plantillas declaradas en el seed, texto blanco solo sobre tarjeta negra, tachado con reflejo total, calendario a 8 fuentes

**Fecha:** 2026-09-14
**Estado:** ✅ Aceptada (decisiones de producto tomadas por Lucy en sesión; la implementación las referencia una a una)

**Contexto:** validando STG pre-salida, Lucy reportó inconsistencias entre lo que el admin edita y lo que el cliente ve: precio tachado sin reflejo en la ficha, polaroid saltando de 10 en 10 unidades, Cuadrados sin stepper, Alargados y Magnéticos cayendo al canvas cuadrado del Estudio, tiras perdiendo el lienzo al agregar unidades, texto preview ilegible según el color de tarjeta, plantillas demo acumuladas en `/admin/plantillas` y solo 3 tipos de letra en el calendario. La investigación encontró causas raíz concretas: las plantillas reales de separadores nunca se declararon en `seed-templates.mjs` (un `--prune` las soft-eliminó), las escaleras históricas de unidades quedaron archivadas en limpiezas previas, y el lazy-mount del Estudio no re-registraba los slots nuevos en el IntersectionObserver.

**Decisión:**
① **Escaleras históricas restauradas, NO aplanadas.** Polaroid 2–9 vuelve con sus precios graduados ($20.200–$33.000, convergiendo al pack de 10 = $34.900; la variante de 2 traía $34.900 por error → corregido a $20.200, interpolación aprobada por Lucy) y Cuadrados 2–6 ($17.600–$24.000), completando Cuadrados 7–10 con el último paso histórico (+$1.500) y TODAS las gemelas `-NOMAG` a precio espejo. Los precios finos se ajustan por variante desde el admin (modelo por opción, D4 histórico).
② **Las plantillas del catálogo se DECLARAN en el seed o no existen.** `seed-templates.mjs` declara las 4 plantillas reales de separadores (sep-mag-2x6/4x4-2, sep-alr-4x12/4x15) y las de tiras con nombre+preview distintos por composición; las globales demo (6) y 2 históricas sin uso se hard-borran con guard de diseños referenciantes (`cleanup-demo-templates.mjs`) y el admin gana acción "Eliminar" (solo no aprobadas y sin diseños). Las dos plantillas reales que quedaban con slug `libre-*` se renombran (el prefijo las marcaba como "respaldo" siendo plantillas de producto).
③ **Texto preview BLANCO solo sobre tarjeta NEGRA.** Se reemplaza la regla de ronda 5 (blanco también sobre rosa/lavanda, umbral 0.56): con tarjeta blanca/aguamarina/rosa/lavanda/amarilla la letra por defecto es el oscuro de la plantilla (umbral 0.30 en `defaultTextFillOnCard`). Misma regla en lienzo, editor de texto y render de producción (WYSIWYG).
④ **Precio tachado con reflejo total.** La promo se edita por opción; el front la muestra siempre: tachado si la opción elegida (o el "Desde") la tiene, chip "Promo en otra opción desde $X" si la tiene otra (`resolvePromoDisplay`). Nunca tachado falso ni descuento negativo.
⑤ **Vista previa bloqueada uniforme.** Ningún producto abre vista previa ni finaliza con fotos requeridas vacías (gate único `selectIsComplete` + bloqueos por superficie, ya arquitectónico — se blinda con test que agregar unidades re-bloquea).
⑥ **Calendario a 8 tipos de letra** (fredoka, inter, caveat + baloo2, nunito, patrick, playfair, dancing — OFL): cada key con su TTF registrado en el render de servidor (`assets/fonts`) y su CSS var de next/font para el canvas del cliente; lista blanca key→familia intacta (nunca string libre del cliente).

**Consecuencia:** los 3 ambientes quedan homologados también en plantillas y escaleras (no solo en productos/variantes — ADR-099); el seed de plantillas vuelve a ser fuente de verdad completa del catálogo de plantillas; la auditoría `audit-storefront-consistency.mjs` queda como chequeo repetible de estas clases de bugs (escaleras, promo denormalizada, cobertura de plantilla por aspect, sanidad de nombres/previews). La anomalía de precios de Magnéticos (5 unid. > 6 unid.) queda REPORTADA a Lucy — es dato curado suyo, no se toca por script.

---

## ADR-101 — Paquete coherencia catálogo/estudio/admin (2026-09-15): Fotoimanes por packs de 6, precio total vivo en PDP, placeholder-guide en el lienzo, 3D a tamaño real siempre, admin consolidado

**Fecha:** 2026-09-15
**Estado:** ✅ Aceptada (4 decisiones de producto confirmadas por Lucy en sesión antes de codificar)

**Contexto:** validando el flujo completo, Lucy reportó un paquete de inconsistencias: el tachado no se veía en la PDP (sí en las cards), el Precio Total no se calculaba al sumar Unidades (Calendario Set 12, Tiras, Abecedario, Nombre Personalizado), las delimitaciones de los slots solo aparecían al arrastrar, las fotos de baja resolución solo se rechazaban sin alternativa, "Fichas del abecedario" colgaba suelto en el menú admin, las plantillas del admin no reflejaban las del Estudio (y las Magnéticas 2×6/4×4.2 se veían horizontales), y la vista 3D encogía las piezas al aumentar unidades (12 tiras = miniaturas) y sobreponía los separadores Alargados.

**Decisión:**
① **Fotoimanes se vende por PACKS de 6 unidades, un producto por formato.** "Fotoimanes Polaroid" (formato único 6×8 cm) y "Fotoimanes Cuadrados" (Tamaño 4×4/5×5/7×7 × packs) quedan como los dos productos de la categoría; el estilo Polaroid Clásica/Instagram se elige como plantilla del Estudio. Variantes = photoSlots 6/12/18/24 con precio lineal de la tabla histórica (Polaroid $3.750/und; Cuadrados $4.000/$4.500/$6.000 por tamaño). La PDP y el Estudio nombran "Packs" con desglose ("1 pack = 6 unidades") y el Estudio carga 6 lienzos por pack. Las variantes legacy (Set 6·7×9, Set 9, Set 20 mini, 9-unidades, cantidades libres 1..10 y gemelas -NOMAG de estas dos familias) se DESACTIVAN (soft-delete, nunca hard-delete: historial intacto) vía `one-shot/fotoimanes-packs-6-20260915.mjs`; el seed canónico declara el modelo nuevo. Mismo sustantivo "Packs" para Abecedario (1 pack = abecedario completo, 27 fichas ES / 26 EN) y Pack Vocales (1 pack = 5 vocales). Los guards del front degradan al conteo en unidades si el catálogo aún es legacy (migración pendiente en un ambiente).
② **Precio vivo y tachado con paridad PDP↔card.** El stepper de unidades de composición fija muestra el total vivo (unitario × N) en la PDP — antes el ×N solo aparecía en la modal de confirmación y el carrito; el fallback de precio por variante sin `price` propio cae al mínimo de la familia (mismo criterio que la card), no al `basePrice` stale; el badge -% acompaña al tachado en la PDP. El control de Unidades del Estudio es universal (todo producto, tope `maxUnitsForProduct`).
③ **El lienzo guía sin ensuciar la producción.** Los slots vacíos muestran delimitación punteada sutil SIEMPRE (morado 0.35), media al hover (0.6) y turquesa pleno al arrastrar — colores centralizados en `studio-brand.ts`. El texto placeholder de CUALQUIER plantilla se dibuja en la grilla atenuado (40%) como `placeholder-guide` (se excluye de snapshots/producción reusando el ocultamiento de `.edit-indicator`); fuera de la grilla sigue la regla Ola 25 (invisible) y producción imprime solo `override.text`.
④ **La foto se mejora en línea, no solo se rechaza.** Upscale local en el navegador (pasos ≤×2 con smoothing high + unsharp leve, tope ×4, sin servicios externos — CSP intacta) antes de subir; si aun así queda bajo el mínimo, banner con copy CMS `estudio.fotos.aviso-mejora-auto`.
⑤ **3D a tamaño REAL siempre.** `magnetWorldSizes` no encoge: pieza = cm × uPerCm fijo; el clúster crece en filas/columnas (`fridgeClusterLayout`/`boardClusterLayout`) y `FitCamera` encuadra nevera + clúster. Alargados en el libro: separación = ancho de pieza + 0.15 u con filas balanceadas en z (sin solape), y la galería "en tu espacio" respeta `flat`/noFold (no rota ni dobla Alargados).
⑥ **Admin consolidado y espejo del Estudio.** "Fichas del abecedario" vive como pestaña de "Diseños prediseñados" (`/admin/disenos?tab=fichas`, redirect 308 desde `/admin/fichas`); `/admin/plantillas` se agrupa por categoría → producto con marca "Visible/No visible en el Estudio" calculada con la MISMA regla del Estudio (reusa `template-visibility.ts`). Las plantillas legadas ola3 de separadores (stage volteado 600×200 y duplicada 400×420) se archivan; los mockups `sep-mag-2x6.svg`/`sep-mag-4x4-2.svg` se regeneran con el aspect real; `diagnose-template-visibility-20260915.mjs` + `normalize-template-visibility-20260915.mjs` quedan como chequeo/corrección repetible por ambiente.

**Consecuencia:** la categoría Fotoimanes queda con 2 productos de compra simple (packs) y el cobro server-side (`photo-pack-resolve`) resuelve 1 variante exacta por combinación — sin cambios en la capa de cobro. Diseños guardados con composición legacy (9/20 unidades) fallarán con `NO_DEFAULT_VARIANT` al agregarse al carrito (comportamiento deliberado ya existente). La migración de datos quedó APLICADA en local; STG/PRD requieren correr los dos one-shots con su `.env` (DRY-RUN por defecto, env-guard fail-closed). Los textos nuevos del Estudio quedaron declarados en el mapa CMS (`estudio.fotos.aviso-mejora-auto`).

---

## Regla de branches (owner 2026-09-15)
