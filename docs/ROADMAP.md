...
| **F** | Refund + Cupones (redención en checkout, reembolso admin) | Fase 4/5 | 🟡 En curso — **F1 cupones LISTO** (redención en checkout: validación de todas las reglas del modelo + aplicación atómica en la orden + `CouponUsage`/`usedCount` idempotente en PAID + UI + 22 tests). Falta **F2 reembolso admin** (state machine + revert stock ya existen; falta acción/botón/email + Wompi manual) y **F3 retracto** (Ley 2439: modelo `RetractRequest` + UI cliente/admin + emails) |
