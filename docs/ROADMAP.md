...
| **F** | Refund + Cupones (redención en checkout, reembolso admin) | Fase 4/5 | 🟡 En curso — **F1 cupones LISTO** (redención en checkout: validación de todas las reglas + aplicación atómica + `CouponUsage`/`usedCount` idempotente en PAID + UI + 22 tests). **F2 reembolso admin LISTO** (campos auditoría + `refundOrder` idempotente con revert de stock + email + acción/botón admin con confirmación + display; dinero Wompi **manual**; tests refund + idempotencia). **F3 retracto** (Ley 2439) **backend LISTO** (modelo `RetractRequest` + `Order.deliveredAt` + servicio elegibilidad/ventana días hábiles/creación + 15 tests); falta la **UI de F3** (botón cliente en mi-cuenta + gestión admin aprobar/recibir/reembolsar + emails) |
