...
| **E** | Testing (RLS, E2E, visual, a11y) | transversal / Fase 7 | 🟡 En curso (~**1.529 vitest** en 4 lotes de servicio/lib + **83 de UI (componentes)** con revisión adversarial; **CI-DB LISTO** — todo corre contra Postgres real y se enforza en cada PR (antes: 0); **E2E Playwright 17/17** (smoke + compra + login admin + **reto MFA TOTP/código de respaldo** + **Estudio Konva**) + **6 bugs reales arreglados**; runner local serial estable (next dev + pooler no toleran concurrencia; CI paralelo contra build prod). Falta: a11y con axe (dep por aprobar), E2E envío/pago (deps externas Aveonline/Wompi), visual, load) |
