...
| **D** | Observabilidad (dashboard, alertas, SLOs) | Fase 7 | 🟡 Backend LISTO — **captura de errores del servidor en DB** (`ErrorLog` vía `instrumentation.onRequestError`, sin Sentry, verificado end-to-end) + **captura de errores del CLIENTE** (`ErrorReport` deduplicado por fingerprint vía `/api/log-error` desde los error boundaries, ADR-047) + **panel `/admin/observability`** (salud técnica: errores servidor+cliente/webhooks/reconciliación/reversas/vitals) + **alertas por email** (`/api/cron/alerts` con dedup 30min, reglas "qué se rompió + qué hacer"). **ACCIÓN HUMANA:** agendar pg_cron + set `CRON_SECRET` (ver OPERATIONS.md). Falta: SLOs cuantitativos + resumen diario 8am (mejoras) |
