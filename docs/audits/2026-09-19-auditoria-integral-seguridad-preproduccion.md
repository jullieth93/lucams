...
**Wompi STG — CERRADO (2026-09-20):** URL de eventos del merchant de pruebas actualizada con `?x-vercel-protection-bypass=<token>` por el propietario. Validación end-to-end con evento firmado real (script `tmp/test-wompi-webhook-stg.mjs`, referencia inexistente para no tocar órdenes): HTTP 200, firma aceptada, evento persistido y procesado en `WebhookEvent` de STG. El webhook STG queda operativo detrás del SSO.

**Vigilante de dominio — IMPLEMENTADO Y OPERANDO (2026-09-20, L-N1/L-H3):** workflow `domain-watch.yml` (diario 11:17 UTC + dispatch manual) consulta el RDAP de Verisign y POSTea la observación a `/api/cron/domain-watch` (CRON_SECRET, timing-safe, fail-closed — verificado en vivo: 401 sin secreto). La app guarda baseline en `AlertState` y alerta por email crítico ante: expiración cruzando 60/30/14/7/3/1 días (+recordatorio diario ≤3 días), cambio de nameservers o de status (p.ej. `client hold` = suspensión ICANN), y 2 fallas seguidas de RDAP. Dead-man propio vía `cron_stale_domain-watch` (48 h). Nota de diseño: Verisign/rdap.org rechazan HTTPS desde los IPs de Supabase/pg_net (verificado en vivo), por eso el productor es GitHub Actions. Certificación: dispatch manual run 35482749118 success → baseline sembrado en PRD (expira 2027-07-19, NS Cloudflare, transfer lock activo, 302 días, 0 alertas). Migración 039 aplicada en STG/PRD/LOCAL.

**R2 Bucket Lock — CERRADO (2026-09-20, por el propietario con captura como evidencia):** regla `lock-backups-29d` (prefix vacío = bucket completo, Age 29 días, **Enabled**) en `lucams-backups`, según la [doc oficial de Bucket locks](https://developers.cloudflare.com/r2/buckets/bucket-locks/) (consultada 2026-09-20). Los backups cifrados no pueden borrarse ni sobrescribirse antes de 29 días — ni siquiera con el API token de R2 (anti-ransomware). El lock tiene precedencia sobre la poda: el valor 29 días deja 24 h de margen contra `BACKUP_KEEP=30` (la copia podada cada noche tiene ~30-31 días) — si el job de backup llegara a fallar por retención, bajar a 25 días. Nota operativa documentada: el bucket no puede vaciarse con reglas activas (quitar reglas primero ante una migración de proveedor).

## ADDENDUM 5 — Batería dinámica §58 ejecutada en STG (2026-09-20)

Matriz ejecutada contra STG (Vercel SSO bypass + Supabase STG) con las 7 cuentas QA, ~60 requests de bajo volumen, fixtures marcados `qa-58-*` con limpieza verificada (conteos en 0). **30+ casos, TODOS PASS. Cero hallazgos nuevos.**

- **Anónimo:** /admin → 307 login; Server Action admin sin sesión → 307; /api/cron sin secreto → 401; webhooks sin firma → 401 (Aveonline → 503 fail-closed, ver nota); export CSV sin sesión → 307; health público sin detalle; rate limit /rastrear → bloqueo al intento 11.
- **IDOR cliente A/B:** archivar/compartir diseño ajeno, leer pedido ajeno, borrar dirección ajena → todos rechazados (`{"ok":false}` / 404, DB intacta); /pedido/[token] alterado → 404 sin oráculo; /rastrear con email mismatch → mensaje genérico.
- **RBAC:** MANAGER→acción SUPER (denied), CMS_EDITOR→finanzas (307 denied), FULFILLMENT→usuarios (307 denied), MANAGER→finanzas (307 denied), admin sin TOTP → enrolamiento forzado (B-1), **admin inactivo con sesión válida → rechazado** (controlado que la sesión Auth era válida: el rechazo es por isActive=false).
- **AAL1/AAL2 (TOTP enrolado programáticamente):** reembolso con aal2 de 14,5 min → `reauthRequired` sin escribir (F-10/F-05 verificado en vivo); con aal2 fresca → REFUNDED + confirmación de dinero + AdminActionLog.
- **Concurrencia (mismo SQL atómico de la app, 2 requests paralelos):** cupón maxUses=1 → 1 ganador (`usedCount=1/1`); tope por cliente → trigger 23505; última unidad stock=1 → `stock=0` sin negativo; doble consumo de recovery code → 1 solo consumo.
- **Webhooks Wompi STG:** firma inválida → 401 + **fila SecurityEvent** (F-07 funcionando en vivo); replay → dedup "already processed" (1 sola fila); timestamp -30h → 401 + SecurityEvent; `environment:"prod"` contra STG → 401 environment mismatch.
- **Transiciones:** DELIVERED→PENDING → "Transición ilegal" (orden intacta); orderId inexistente → error genérico; control PENDING_PAYMENT→CANCELLED → OK.
- **CSV:** CMS_EDITOR → 307 denied; MANAGER → 200 con celda `=2+5` cruda → **L-F3 confirmado en vivo** (hallazgo ya conocido del ledger — pendiente su fix menor: prefijo `'` en `csvCell`).
