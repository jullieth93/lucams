...
| F-5 Cadena dev hono/qs | ✅ | Overrides `hono@^4.12.21`, `@hono/node-server@^1.19.15`, `qs@^6.15.2`; `pnpm audit` full ya no los reporta. **Extra detectado y cerrado (2026-08-29, commit `6fd6140`):** los ~35 advisories dev-only adicionales quedaron triageados con overrides dirigidos (undici, vite, brace-expansion, js-yaml, fast-uri, ip-address, body-parser, tmp, uuid) — audit full pasa de 35 a **1** (extract-zip: el parche 2.0.2 aún no existe en npm; documentado en `pnpm-workspace.yaml` para reintento). |
...
5. **~35 advisories dev-only adicionales** en `pnpm audit` full (undici, vite, brace-expansion, js-yaml, etc. vía vitest/jsdom/playwright/shadcn) → **cerrado el 2026-08-29** (`6fd6140`): overrides dirigidos dejan el audit full en **1** advisory (extract-zip, parche aún no publicado en npm — nota en `pnpm-workspace.yaml`). Además: `lib/error-capture.integration.test.ts` tiene un flake de paralelismo preexistente (aserción inmediata tras escritura async a DB compartida; verde en aislamiento y en re-run) — candidato a `waitFor`/retry en una tarea de higiene de tests.
...
4. **Supabase Dashboard (prod y stg):** 📌 **diferido por decisión del operador (2026-08-29):** activar Leaked Password Protection (V2-6) solo cuando el proyecto pase a plan pago — queda como pendiente de ese momento. Mientras tanto el control propio de la app (HIBP k-anonymity en registro/cambio de password, `lib/pwned-passwords.ts`) ya cubre los flujos propios.
